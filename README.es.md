...
| **Backend** | Node.js + Express 5, organizado por capas (routes / controllers / services) |
...
```
app-familia/
├── backend/                    API REST + WebSocket, organizada por capas
│   ├── index.js                  Arranque: Express, middlewares globales, monta los routers
│   ├── socket.js                  Handler de Socket.IO (conexión, salas por familia)
│   ├── db/
│   │   └── conexion.js              Conexión SQLite, esquema y migraciones
│   ├── middleware/
│   │   ├── auth.js                   Verificación de JWT
│   │   ├── familia.js                 Pertenencia a familia / "solo tú mismo"
│   │   └── validar.js                 Adapta un esquema de Zod a middleware de Express
│   ├── schemas/                    Esquemas de validación de Zod, uno por recurso
│   ├── routes/                     Endpoints agrupados por recurso
│   ├── controllers/                 Validan la orquestación y arman la respuesta
...
Cada recurso del backend (auth, tareas, familias, chat, mascota, notificaciones) sigue el mismo patrón: `routes/` define los endpoints y aplica los middlewares de autenticación/validación, `controllers/` extrae los datos de la petición y arma la respuesta, y `services/` contiene toda la lógica de negocio y las consultas a la base de datos. Un endpoint que necesita el trabajo de varios dominios a la vez (por ejemplo, completar una tarea afecta a puntos, salud de la mascota y notificaciones) lo resuelve el servicio del recurso principal llamando a los servicios de los demás — nunca se repite la lógica.
...
- [ ] Tests automatizados para los servicios del backend
